...
export interface ListarModelosResponse {
  modelos: ModeloInfo[]
  total: number
  nota: string
}

export interface CarregarModeloResponse {
  nome: string
  carregado: boolean
  descricao: string
  num_camadas: number
...
export interface RealAttentionCamada {
  camada: number
  cabecas: { cabeca: number; pesos: number[][] }[]
  media_cabecas: number[][]
}

export interface RealAttentionResponse {
  modelo: string
  texto: string
  tokens: string[]
  num_tokens: number
  camadas: RealAttentionCamada[]
  explicacao: string
}
...
export interface HardwareInfo {
  ...
  nucleos: number
  ram_total_gb: number
  ram_disponivel_gb: number
  gpu: string | null
  gpu_disponivel: boolean
  sistema: string
  python_version: string
  torch_instalado: boolean
  torch_version: string | null
  transformers_instalado: boolean
...
export interface AutoConfigureResponse {
  sucesso: boolean
  modelo_nome: string
  modelo_info: CarregarModeloResponse | null
  hardware: HardwareInfo
  recomendacao_razao: string
  etapas: {
    etapa: string
    status: 'pendente' | 'em_andamento' | 'completo' | 'erro'
    detalhe: string
  }[]
  erro: string | null
}
